...
BEGIN
    SELECT c.id_curso, c.nombre, c.descripcion, c.duracion
    FROM Cursos c
...
BEGIN
    ...
    JOIN Inscripciones i ON c.id_inscripcion = i.id_inscripcion
    SET c.nota = p_nueva_nota, c.fecha_evaluacion = p_fecha_evaluacion
    WHERE i.id_estudiante = p_id_estudiante AND i.id_curso = p_id_curso;
END //
...
CALL InscribirEstudiante(5, 5, '2025-03-28');

CALL CalcularPromedioEstudiante(1, @promedio);
SELECT @promedio;

CALL ObtenerCursosEstudiante(1);

CALL ActualizarCalificacion(1, 1, 92.0, '2025-03-17');
select * from Calificaciones;

CALL EliminarInscripcion(1);
